...
def updatestudentcourses(ctx, fromSubjectNo, ToSubjectNo):
    ...
    if checkstudentenroled(fromSubjectNo, user_name):
    ...
    student_col = ToSubjectNo
    student_row = interface.get_row_index(user_name, student_col)
    if not (checkstudentenroled(ToSubjectNo, user_name)):
...
def checkstudentenroled(student_col, student_name):
